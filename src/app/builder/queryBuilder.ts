...
class QueryBuilder<T> {
  ...
  search(searchableFields: string[]) {
    ...
    // ObjectId exact match search
    if (Types.ObjectId.isValid(searchTerm)) {
      orConditions.push(
        { _id: new Types.ObjectId(searchTerm) } as FilterQuery<T>
      );

      orConditions.push(
        { bookingId: new Types.ObjectId(searchTerm) } as FilterQuery<T>
      );
    }
  ...
  filter() {
    ...
    if (queryObj.status === "HISTORY") {
      ...
      this.modelQuery = this.modelQuery.where("status").in([
        "CANCELLED",
        "COMPLETED",
      ]);
    }
